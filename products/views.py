...
from log.utils import (
    attribute_creation_log,
    attribute_delation_log,
    product_create_log,
    product_deletion_log,
    product_update_log,
)
...
def add_color(request):
    ...
    if not is_user_admin(user) or is_user_moderator(user):
        return JsonResponse(
            {"error": "Unauthorized. Admin  or moderator access required."}, status=403
        )
...
def add_size(request):
    ...
    if request.method == "POST":
        ...
        return JsonResponse(
            {"message": "Size added successfully.", "size_id": str(result.inserted_id)},
            status=201,
        )
...
def add_subcategory(request, category_id):
    ...
    try:
        ...
        existing_subcategory = subcategories_col.find_one(
            {"name": subcategory_name.lower(), "parent_id": ObjectId(category_id)}
        )
...
def list_subcategories(request, category_id):
    ...
    try:
        ...
        if not category:
            return JsonResponse({"error": "Category not found."}, status=404)
        subcategories = list(
            subcategories_col.find({"parent_id": ObjectId(category_id)})
        )
...
def delete_category(request, category_id):
    ...
    if request.method == "DELETE":
        ...
        products_col.update_many(
            ...
            {"$set": {"category_id": "undefined", "subcategory_id": "undefined"}},
        )
...
def delete_subcategory(request, subcategory_id):
    ...
    if request.method == "DELETE":
        ...
        return JsonResponse(
            {"message": "Subcategory deleted successfully."}, status=200
        )
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if not name or not description or price <= 0 or stock < 0:
                return JsonResponse(
                    {"error": "Please provide valid product details."}, status=400
                )
            if not subcategory_id:
                return JsonResponse(
                    {"error": "Subcategory ID is required."}, status=400
                )
            ...
            if not images or len(images) > 3:
                return JsonResponse(
                    {"error": "Please upload between 1 to 3 images."}, status=400
                )

            if not color_ids or not size_ids:
                return JsonResponse(
                    {"error": "Color and size are required"}, status=400
                )
            if discount:
                if discount <= 0 or discount >= 1:
                    return JsonResponse(
                        {"error": "Discount must be between 0 and 1."}, status=400
                    )
            ...
            if subcategory_id:
                ...
                subcategory = subcategories_col.find_one(
                ...
                if not subcategory:
                    return JsonResponse(
                        {
                            "error": "sub category not found or does not belong to the category"
                        }
                    )
...
def get_product_details(request, product_id):
    try:
        ...
        product["category_id"] = (
            str(product["category_id"]) if product.get("category_id") else None
        )
        product["subcategory_id"] = (
            str(product["subcategory_id"]) if product.get("subcategory_id") else None
        )
        ...
        product["created_at"] = (
            product["created_at"].isoformat() if product.get("created_at") else None
        )
        product["updated_at"] = (
            product["updated_at"].isoformat() if product.get("updated_at") else None
        )
...
def update_product(request, product_id):
    try:
        ...
        if request.method == "PUT":
            ...
            if not update_data:
                return JsonResponse(
                    {"error": "No fields provided to update"}, status=400
                )
        ...
        # ---------- GET PRODUCT ----------
            ...
            product["_id"] = str(product["_id"])
            product["category_id"] = (
                str(product.get("category_id")) if product.get("category_id") else None
            )
...
def update_product_images(request, product_id):
    try:
        ...
        try:
            ...
            remove_images = [
                s.strip() for s in remove_images_str.split(",") if s.strip()
            ]
        ...
        if len(add_images) > 3:
            return JsonResponse(
                {"error": "You can upload maximum 3 images"}, status=400
            )
        ...
        if not (1 <= len(existing_images) <= 3):
            return JsonResponse(
                {"error": "Product must have between 1 and 3 images"}, status=400
            )
